```dataviewjs
// 购物金额 Year-over-Year Comparison
...
for (let page of pages) {
    ...
    if (match) {
        ...
        if (year === currentYear && month >= 0 && month < 12) {
            shopping_currentYear[month] = page.购物金额;
        } else if (year === lastYear && month >= 0 && month < 12) {
            shopping_lastYear[month] = page.购物金额;
        }
    }
}
...
const chartData = {
    ...
    options: {
        ...
        plugins: {
            title: { display: true, text: '购物金额 年度对比' },
            legend: { position: 'top' }
        },
        scales: {
...
window.renderChart(chartData, chartDiv);
    ...
    console.error('Shopping Chart Error:', error);
